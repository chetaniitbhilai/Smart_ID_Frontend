...
const HolidaysComponent = ({ attendanceData }) => {
  // Group attendance records by date
  const groupByDate = (data) => {
    const grouped = {};
    
    data.forEach(record => {
      const date = record.month;
      if (!grouped[date]) {
        grouped[date] = [];
      }
    ...
    <div className="attendance-history-page">
      ...
      <div className="attendance-content">
        {Object.keys(groupedAttendance).map(month => (
          <div key={month} className="date-section">
            ...
            {groupedAttendance[month].map((record, index) => (
              <div key={index} className="attendance-card">
                <div className="attendance-details">
                  <div className="attendance-time">{record.date}</div>
                  <div className="attendance-location"></div>
                  <div className="attendance-subject">
                    <div className="subject-dot" style={{ backgroundColor: record.subjectColor || '#FFA500' }}></div>
                    <span>{record.festival}</span>
                  </div>
                </div>
